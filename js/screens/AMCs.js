...
const paymentTypeList = ['Type of payment'];
const cityList = ['City'];
const bankList = ['Bank'];
const mfrList = ['MFR'];
export default class AMCs extends Component {
    constructor(props) {
        super(props);
        this.state = {
            email: "",
    ...
    getamcs() {
        ...
        this.setState({ paymentTypeList: paymentTypeList });
        this.setState({ cityList: cityList });
        this.setState({ bankList: bankList });
        this.setState({ mfrList: mfrList });
    }
    ...
    filteritem() {
        const newData = this.state.calls_data.filter((mydata) => {
            const paymenttype = (this.state.paymenttype == 'Type of Payment') ? '' : this.state.paymenttype.toUpperCase();
            const city = (this.state.city == 'City') ? '' : this.state.city.toUpperCase();
            const bank = (this.state.bank == 'Bank') ? '' : this.state.bank.toUpperCase();
            const mfr = (this.state.mfr == 'MFR') ? '' : this.state.mfr.toUpperCase();

            const result = mydata;
            if (paymenttype != '') {
                if (mydata.paymenttype.toUpperCase().indexOf(paymenttype) < 0) {
                    return false;
                }
            }
            if (city != '') {
            ...
            if (mfr != '') {
                if (result.mfr.toUpperCase().indexOf(mfr) < 0) {
                    return false;
                }
            }
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1 }}>
                ...
                <MenuHeader title="Your AMC's" navigation={this.props.navigation} />
                    ...
                    <View style={styles.tableview}>
                        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                            <View style={styles.tablehead}>
                                ...
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Total AMC Value</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Balance Due</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Type of Payment</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Start Date</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>End Date</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Due Days</Text>
                                </View>
                            </View>
                        </ScrollView>
                        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                ...
                <View style={[styles.sheet]}>
                    <Animated.View style={[styles.popup, slideUp]}>
                        <View style={{ marginBottom: 10 }}>
                            ...
                            <View style={{ backgroundColor: '#f2f2f2', marginTop: '3%' }}>
                                <Picker
                                    selectedValue={this.state.mfr}
                                    style={styles.picker}
                                    mode={'dropdown'}
                                    onValueChange={(itemValue, itemIndex) =>
                                        this.setState({ mfr: itemValue })
                                    }>
                                    {this.state.mfrList.map((row, index) => (
                                        <Picker.Item label={row} value={row} key={index} />
                                    ))}
                                </Picker>
                            </View>
                            ...
                            <View style={{ backgroundColor: '#f2f2f2', marginTop: '3%' }}>
                                <Picker
                                    ...
                                    {this.state.paymentTypeList.map((row, index) => (
                                        <Picker.Item label={row} value={row} key={index} />
                                    ))}
                                </Picker>
                            </View>
