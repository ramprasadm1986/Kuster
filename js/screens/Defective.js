...
const bankList = ['Bank'];
const partsGroupList = ['Parts Group'];
export default class Defective extends Component {
    constructor(props) {
        super(props);
        this.state = {
            email: "",
    ...
    getdefective() {
        ...
        this.setState({ partsGroupList: partsGroupList });
    }
    ...
    filteritem() {
        const newData = this.state.calls_data.filter((mydata) => {
            ...
            const partsgroup = (this.state.partsgroup == 'MFR') ? '' : this.state.partsgroup.toUpperCase();
            ...
            if (partsgroup != '') {
                if (result.partsgroup.toUpperCase().indexOf(partsgroup) < 0) {
                    return false;
                }
            }
    ...
    render() {
        ...
        return (
            <Container style={{ flex: 1 }}>
                ...
                <MenuHeader title="Defective Parts" navigation={this.props.navigation} />
                    ...
                    <View style={styles.tableview}>
                        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                            <View style={styles.tablehead}>
                                ...
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Parts Group</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>Quantity</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>IT Date</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>IT Number</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>ITR Date</Text>
                                </View>
                                <View style={{ width: 150 }}>
                                    <Text style={styles.tableheadtext}>ITR Number</Text>
                                </View>
                            </View>
                        </ScrollView>
                        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                ...
                <View style={[styles.sheet]}>
                    <Animated.View style={[styles.popup, slideUp]}>
                        <View style={{ marginBottom: 10 }}>
                            ...
                            <View style={{ backgroundColor: '#f2f2f2', marginTop: '3%' }}>
                                <Picker
                                    selectedValue={this.state.partsgroup}
                                    style={styles.picker}
                                    mode={'dropdown'}
                                    onValueChange={(itemValue, itemIndex) =>
                                        this.setState({ partsgroup: itemValue })
                                    }>
                                    {this.state.partsGroupList.map((row, index) => (
                                        <Picker.Item label={row} value={row} key={index} />
                                    ))}
                                </Picker>
                            </View>
